                ...
                <div class="row mb-1">
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-left">
                            <li class="breadcrumb-item"><h5><a href="{{ route('products.pesq', ['pesqdefault' => 'index']) }}">Voltar</a></h5></li>
                            <li class="breadcrumb-item active">Alteração de Produtos</li>
                        </ol>
                    </div>
                </div>
            </div>
                                    ...
                                    <form action="{{ route('products.update', ['product' => $Product->id]) }}" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="form-row">
                                            <div class="col">
                                                ...
                                                <input type="hidden" name="store" id="idstore" value="{{$Product->store}}">
                                            ...
                                            <div class="col">
                                                ...
                                                <select id="idcategoryproduct" name="category_product" data-placeholder="selecione uma categoria..." class="chosen-select">
                                                    ...
                                                    @foreach($listCategoriesProduct as $categoriesproduct)
                                                        ...
                                                        <?php echo(($Product->category_product == $categoriesproduct->id) ? "selected" : "")?>
                                            ...
                                            <div class="col col-2">
                                                ...
                                                <select id="idstatus" name="status" class="custom-select d-block w-100">
                                                    <option value="S"
                                                            {{(($Product->status == 'S') ? "selected" : "")}}>Sim
                                                    </option>
                                                    <option value="N"
                                                            {{(($Product->status == 'N') ? "selected" : "")}}>Não
                                                    </option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="form-row">
                                            <div class="col">
                                                <?php $teste = 'store' ?>
                                                <label for="inputKit">Kits:</label>
                                                <select id="idkit" name="kit[]" data-placeholder=" selecione um kit se o produto pertencer a um kit..." class="chosen-select" multiple tabindex="4">
                                                    <option value=""></option>
                                                    @foreach($listKit as $Kit)
                                                        @if($teste !== $Kit->store)
                                                            <optgroup label="{{ $Kit::find($Kit->id)->pesqStore->name }}">
                                                                @endif
                                                                <option value="{{$Kit->id}}"
                                                                @foreach($listRelKitsProducts as $RelKitsProducts)
                                                                    {{(($RelKitsProducts->kit == $Kit->id) ? "selected" : "")}}
                                                                        @endforeach
                                                                >{{$Kit->name}}</option>
                                                                {{ $teste = $Kit->store }}
                                                                @if($teste !== $Kit->store)
                                        ...
                                        <div class="form-row">
                                            <div class="col">
                                                ...
                                                <input value="{{$Product->id_pdv_store}}" type="text" class="form-control" id="idpdv"  name="id_pdv_store" >
                                            </div>
                                            <div class="col">
                                                <label for="inputCodigopdv">Código do pdv:</label><span class="text-danger col-1">{{$errors->first('codigo_pdv_store')}}</span>
                                                <input value="{{$Product->codigo_pdv_store}}" type="text" class="form-control" id="idcodigopdv"  name="codigo_pdv_store" >
                                            </div>
                                            <div class="col">
                                                ...
                                                <input value="{{$Product->codigo_barras_pdv_store}}" type="text" class="form-control" id="idcodigobarraspdv"  name="codigo_barras_pdv_store" >
                                            </div>
                                        </div>
                                        <div class="form-row">
                                            <div class="col">
                                                <label for="inputName">Nome do Produto:</label><span class="text-danger col-1">{{$errors->first('name')}}</span>
                                                <input value="{{$Product->name}}" type="text" class="form-control" id="idname"  name="name" >
                                            </div>
                                        </div>
                                        <div class="form-row">
                                            <div class="col">
                                                <label for="inputAmount">Quantidade:</label><span class="text-danger col-1">{{$errors->first('amount')}}</span>
                                                <input value="{{$Product->amount}}" type="text" class="form-control" id="idamount"  name="amount">
                                            </div>
                                            <div class="col">
                                                <label for="inputUnitprice">Preço:</label><span class="text-danger col-1">{{$errors->first('unit_price')}}</span>
                                                <input value="{{$Product->unit_price}}" type="text" class="form-control" id="idunitprice"  name="unit_price">
                                            </div>
                                        </div>
                                        <div class="form-row">
                                            <div class="col">
                                                ...
                                                <input value="{{$Product->unit_promotion_price}}" class="form-control" name="unit_promotion_price" type="text" id="idunitpromotionprice" >
                                            </div>
                                            <div class="col">
                                                <label for="inputUnitdiscount">Percentual disconto:</label><span class="text-danger col-1">{{$errors->first('unit_discount')}}</span>
                                                <input value="{{$Product->unit_discount}}" class="form-control" name="unit_discount" type="text" id="idunitdiscount">
                                            </div>
                                        </div>
                                        <div class="form-row">
                                            <div class="col">
                                                <label for="inputDescription">Descrição do Produto:</label><span class="text-danger col-1">{{$errors->first('description')}}</span>
                                                <textarea name="description" id="iddescription" class="md-textarea form-control" rows="4">{{$Product->description}}</textarea>
                                            </div>
                                        </div>
                                        <div class="input-field">
                                            <label class="active">Imagens do Produto:</label>
...
@section('javascript')
    ...
    <script>
        ...
        let preloaded = [
            @foreach($relImagensProduct as $ImagensProduct)
                @if(!is_null($ImagensProduct->path_image))
                    {
                        id: '{{$ImagensProduct->id}}',
                        src: '{{ $pathImagens }}/products/store_id_{{ $ImagensProduct->store }}/{{$Product->id}}/small/{{ $ImagensProduct->path_image }}'
                    },
                @endif
            @endforeach
        ];

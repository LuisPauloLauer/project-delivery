        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form name="formDeleteObject" action="{{ route('categoriesstore.delete') }}" method="post">
                        @csrf
                        @method('DELETE')
                        <input type="hidden" name="categoriesStore_id" id="idDeleteObject" value="">
                ...
                <div class="row mb-1">
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-left">
                            <li class="breadcrumb-item"><h5><a href="{{ route('categoriesstore.create') }}">Cadastrar</a></h5></li>
                            <li class="breadcrumb-item active">Categorias de lojas</li>
                        </ol>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <div class="card">
                            ...
                            <!-- card-body -->
                            <div class="card-body">
                                <table id="idtableCategoriesStore" class="table table-bordered table-striped">
                                    <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>Habilitado</th>
                                    ...
                                    <tbody>
                                    @foreach($listCategoriesStore as $CategoriesStore)
                                        <tr>
                                            <td>{{ $CategoriesStore->id }}</td>
                                            <td>
                                                <input id="ckbstatus" name="ckbstatus" class="ckbstatus-bootstrap-switch" type="checkbox" {{ (($CategoriesStore->status == 'S') ? "checked" : "") }} data-objectid="{{$CategoriesStore->id}}">
                                            </td>
                                            <td>{{ $CategoriesStore->name }}</td>
                                            <td>
                                                <div>
                                                    <a id='idBtnEdit' class="btn btn-large btn-warning" href="{{ route('categoriesstore.edit', ['categoriesstore' => $CategoriesStore->id]) }}"><span class="glyphicon fas fa-edit"></span></a>
                                                    <a id='idBtnDelete' class="btn btn-large btn-danger" data-objectid="{{$CategoriesStore->id}}" data-objectname="{{$CategoriesStore->name}}" href="#"><span class="glyphicon fas fa-trash"></span></a>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
...
@section('javascript')
    ...
    <script>
        $(document).ready(function () {
            ...
            <!-- DataTables -->
            $(function () {
                $("#idtableCategoriesStore").DataTable({
                    "responsive": true,
                    "autoWidth": false,
                });
            });
            ...
            $('.bootstrap-switch-id-ckbstatus').on('switchChange.bootstrapSwitch', function (event, state) {
                ...
                $.ajax({
                    ...
                    url: "{{ route('categoriesstore.change.status') }} ",
                    type: "post",
                    data : {
                        object_id : object_id,
